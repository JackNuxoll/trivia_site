...
// ── Quiz metadata (mirrors quiz/[id]/page.tsx — replace with shared fetch later) ──
...
function Sparkline({ data, highlight }: { data: QuizResult[]; highlight: number }) {
  ...
  function yOf(p: number) { return PAD.top  + innerH - ((p - minPct) / range) * innerH; }
  ...
  const areaPath =
    `${linePath} L ${xOf(data.length - 1).toFixed(1)} ${(PAD.top + innerH).toFixed(1)} L ${PAD.left} ${(PAD.top + innerH).toFixed(1)} Z`;
    ...
    >
      ...
      {gridLines.map((v) => (
        <g key={v}>
          <line
            ...
            y1={yOf(v)}   y2={yOf(v)}
            stroke="#E5E3DC" strokeWidth="1" strokeDasharray="4 3"
          />
          <text
      ...
      {data.map((d, i) => {
        ...
        return (
          <g key={i}>
            <circle
              cx={xOf(i)} cy={yOf(d.pct)} r={isHighlight ? 6 : 4}
              fill={isHighlight ? "#4F46E5" : "#fff"}
              stroke="#4F46E5" strokeWidth={isHighlight ? 2.5 : 2}
            />
            {isHighlight && (
              <text
                x={xOf(i)} y={yOf(d.pct) - 10}
                textAnchor="middle" fontSize="11" fontWeight="700"
                fill="#4F46E5" fontFamily="Sora, sans-serif"
              >
                {d.pct}%
              </text>
            )}
...
const STORAGE_KEY_PREFIX = "quizsharp-results-";

function loadResults(quizId: string): QuizResult[] {
  try {
...
function saveResult(quizId: string, result: QuizResult): QuizResult[] {
  const existing = loadResults(quizId);
  const updated  = [...existing, result];
  try {
    localStorage.setItem(`${STORAGE_KEY_PREFIX}${quizId}`, JSON.stringify(updated));
  } catch {
...
export default function ResultsPage() {
  ...
  // Parse URL params (set by quiz page's handleFinish)
  ...
  // Save this result and load history on mount
  useEffect(() => {
    const thisResult: QuizResult = { score, total, pct, timestamp: Date.now() };
    const all = saveResult(quizId, thisResult);
    setHistory(all);
  ...
  const showTrend = storageReady && history.length >= 2;
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", background: "#F8F7F4", minHeight: "100vh" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .r-nav-cta { padding: 8px 20px; border-radius: 8px; background: #4F46E5; color: #fff; font-size: 14px; font-weight: 600; text-decoration: none; transition: background 0.15s; }
        .r-nav-cta:hover { background: #4338CA; }

        /* ── Shell ── */
        .r-shell {
          max-width: 720px; margin: 0 auto;
          padding: clamp(2rem, 5vw, 3.5rem) clamp(1.5rem, 5vw, 2rem) 5rem;
        ...
        .score-left {}
        .score-big {
          font-family: 'Sora', sans-serif; font-size: clamp(3.5rem, 10vw, 5rem);
          font-weight: 800; color: #1A1A2E; letter-spacing: -3px; line-height: 1;
        }
        ...
        .score-pct {
          font-family: 'Sora', sans-serif; font-size: clamp(2.2rem, 6vw, 3rem);
        ...
        .stat-tile {
          background: #F8F7F4; border-radius: 12px; padding: 1rem 1.25rem;
        ...
        .trend-card {
          ...
          animation: trendIn 0.4s 0.15s cubic-bezier(0.22, 1, 0.36, 1) both;
        }
        .trend-header {
        ...
        .trend-pill {
          ...
          border-radius: 99px; background: #F8F7F4; color: #6B7280;
          border: 1px solid #EFEDE7;
        }
        .trend-pill-dot { width: 6px; height: 6px; border-radius: 50%; }

        .trend-chart-wrap { margin-bottom: 0.5rem; }
        ...
        /* First-attempt shimmer placeholder */
        @keyframes shimmer { from { background-position: -400px 0; } to { background-position: 400px 0; } }
        .shimmer-bar {
          height: 120px; border-radius: 10px;
          background: linear-gradient(90deg, #F8F7F4 25%, #EFEDE7 50%, #F8F7F4 75%);
        ...
        .actions-row {
          ...
          animation: actionsIn 0.4s 0.25s cubic-bezier(0.22, 1, 0.36, 1) both;
        }
        .action-btn {
        ...
        <div className="trend-card">
          ...
          {!storageReady ? (
            ...
            <div className="trend-empty">
              ...
              Complete this quiz again to start tracking your progress over time. Results are shared across all players.
            </div>
          )}
        </div>
