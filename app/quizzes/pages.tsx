...
// ── Sample data (replace with fetch('/api/quizzes') later) ──────────────────
const ALL_QUIZZES: QuizSummary[] = [
  ...
  { id: "geo-flags",       title: "Flags of the World",    category: "Geography",   difficulty: "Medium", questions: 25, plays: 1670 },
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", background: "#F8F7F4", minHeight: "100vh" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .b-logo { font-family: 'Sora', sans-serif; font-size: 18px; font-weight: 700; color: #1A1A2E; letter-spacing: -0.5px; text-decoration: none; }
        .b-logo span { color: #4F46E5; }
        .b-nav-links { display: flex; align-items: center; gap: 8px; }
        .b-nav-link { padding: 8px 16px; border-radius: 8px; font-size: 14px; font-weight: 500; color: #4B5563; text-decoration: none; transition: background 0.15s; }
        .b-nav-link:hover { background: #EEECFF; color: #4F46E5; }
        .b-nav-cta { padding: 8px 20px; border-radius: 8px; background: #4F46E5; color: #fff; font-size: 14px; font-weight: 600; text-decoration: none; }
        .b-nav-cta:hover { background: #4338CA; }
      ...
      {/* ── Nav ── */}
      <nav className="b-nav">
        <Link href="/" className="b-logo">Quiz<span>Sharp</span></Link>
        <div className="b-nav-links">
          <Link href="/leaderboard" className="b-nav-link">Leaderboard</Link>
          <Link href="/login" className="b-nav-cta">Sign in</Link>
        </div>
      </nav>
